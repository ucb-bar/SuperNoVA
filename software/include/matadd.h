...
#define matadd_loop_packed(B0, B1, B2, B3, tile0, tile1, tile2, tile3) \
  { \
    ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_MATADD, ((uint64_t) (B0) << 36) | ((uint64_t) (B1) << 24) | ((uint64_t) (B2) << 12) | ((uint64_t) (B3)), ((uint64_t)(tile3) << 56) | ((uint64_t)(tile2) << 48) | ((uint64_t)(tile1) << 40) | ((uint64_t) (tile0) << 32), k_LOOP_PACK) \
  }
/*
#define matadd_loop_packed(A0, A1, A2, A3, B0, B1, B2, B3, tile0, tile1, tile2, tile3) \
  { \
    ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_MATADD, ((uint64_t) (B0) << 36) | ((uint64_t) (B1) << 24) | ((uint64_t) (B2) << 12) | ((uint64_t) (B3)), ((uint64_t)(tile3) << 59) | ((uint64_t)(tile2) << 54) | ((uint64_t)(tile1) << 49) | ((uint64_t) (tile0) << 44) | ((uint64_t) (A0) << 33) | ((uint64_t) (A1) << 22) | ((uint64_t) (A2) << 11) | ((uint64_t) (A3)), k_LOOP_PACK) \
  }
  */
// flush
...
#else
    //printf("I %d, J: %d\n", I, J);
    tiled_resadd_stride_auto(I, J, 1, 1, 1,
...
static void tiled_packed_matadd(const size_t I, int* J,
    ...
    //printf("config: %d, A_stride: %d, B_stride: %d, C_stride: %d\n", config, A_stride, B_stride, C_stride);
    if(config){
#if MATADD == 1
      matadd_extended_config_st(C_stride * sizeof(elem_t), relu ? RELU : NO_ACTIVATION, C_scale);
      //matadd_config_ex(WS, 0, 0);
    ...
    if(num_J > 1){
      ...
      for(int jj = 0; jj < num_J; jj+=4){
        ...
        for(int j = 0; j < num_blk; j++){
          ...
          //if(j != num_blk - 1) A_offset[j+1] = A_offset[j] + dim_J[j];//*(A_blk + j + jj) - *A_blk;
          B_offset[j] = *(B_blk + j + jj) - *B_blk;
          //if(B_offset[j] >= 4096)
          //    printf("B_offset: %d\n", B_offset[j]);
          //if(dim_J[j] >= 32)
          //    printf("dim_J: %d\n", dim_J[j]);
        ...
        //printf("B_offset: %d, %d, %d, %d\n", B_offset[0], B_offset[1], B_offset[2], B_offset[3]);
        //printf("tile_J: %d, %d, %d, %d, pad_J: %d, %d, %d, %d\n", tile_J[0], tile_J[1], tile_J[2], tile_J[3], pad_J[0], pad_J[1], pad_J[2], pad_J[3]);
        //printf("dim_J: %d, %d, %d, %d, num_blk: %d\n", dim_J[0], dim_J[1], dim_J[2], dim_J[3], num_blk);
#if MATADD == 1
        matadd_loop_packed(B_offset[0], B_offset[1], B_offset[2], B_offset[3], dim_J[0], dim_J[1], dim_J[2], dim_J[3]);
        //matadd_loop_packed(A_offset[0], A_offset[1], A_offset[2], A_offset[3], B_offset[0], B_offset[1], B_offset[2], B_offset[3], dim_J[0], dim_J[1], dim_J[2], dim_J[3]);//, pad_J[0], pad_J[1], pad_J[2], pad_J[3]);
        matadd_loop_ws(tile_I, 0, pad_I, 0, A_addr, B, B, A_stride, B_stride, C_stride, num_blk);
#else
        float* A_addr_res = (float*) A_addr;
        for(int i = 0; i < num_blk; i++){
           float* B_addr = (float*) B + B_offset[i];
           /*(
           tiled_resadd_stride_auto(
                   I, dim_J[i], 1, 1, 1,
                   A_stride, B_stride, C_stride,
                   A_addr_res, B_addr, B_addr,
                   false, WS);
           */
           tiled_resadd_small(I, dim_J[i], 
                   A_stride, B_stride, C_stride, 
                   1, 1, 1, 
                   A_addr_res, B_addr, B_addr, 
                   false, true);
      ...
      //printf("dim_I: %d, dim_J: %d, tile_I: %d, pad_I: %d, tile_J: %d, pad_J: %d\n", I, dim_J, tile_I, pad_I, tile_J, pad_J);
#if MATADD == 1 
      matadd_loop_ws(tile_I, tile_J, pad_I, pad_J, A, B, C, A_stride, B_stride, C_stride, 1);
//rr_fence(add_CFGID);
#else
      /*
      tiled_resadd_stride_auto(I, dim_J, 1, 1, 1, 
              A_stride, B_stride, C_stride,
              A, B, B, false, WS);
      */
      tiled_resadd_small(I, dim_J, A_stride, B_stride, C_stride,
	      1, 1, 1,
              A, B, B, false, true);
      //rr_fence(0);
#endif
