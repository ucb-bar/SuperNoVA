...
static void double_tiled_matmul_outer(size_t dim_I, size_t dim_J, size_t dim_K,
  ...
  if(A_channel == -1){
    ...
    dma_nord_source_config(A_channel, A, stride_A);
    dma_nord_dest_config(A_channel, A_sp_addr, A_spad_stride);
  }
  // configure DMA
  dma_nord_source_config(B_channel, B, stride_B);
  dma_nord_dest_config(B_channel, B_sp_addr, B_spad_stride);
...
static void double_tiled_conv(
  ...
  if(A_channel == -1){
    ...
    dma_nord_source_config(A_channel, input, in_stride);
    dma_nord_dest_config(A_channel, A_sp_addr, A_spad_stride);
  }
  // configure DMA
  dma_nord_source_config(B_channel, weights, weight_stride);
  dma_nord_dest_config(B_channel, B_sp_addr, B_spad_stride);
