...
#define ACC_START 0
#define DMA_START 4//10
#define ADD_START 0//15
...
#define gemmini_loop_ws(I, J, K, pad_I, pad_J, pad_K, A, B, D, C, A_stride, B_stride, D_stride, C_stride, A_transpose, B_transpose, full_C, low_D, ex_accumulate, act, a_spad_id, b_spad_id, is_resadd, ab_same) \
  { \
    ...
    ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, ((uint64_t)(a_spad_id) << 18) | ((uint64_t)(b_spad_id) << 16) | ((uint64_t)(act) << 8) | ((low_D) << 2) | ((full_C) << 1) | (ex_accumulate), ((ab_same) << 3) | ((is_resadd) << 2) | ((B_transpose) << 1) | (A_transpose), k_LOOP_WS) \
  }
...
static void tiled_matmul_small(size_t dim_I, size_t dim_J, size_t dim_K,
  ...
  const size_t tile_I = dim_I_padded / DIM;
  const size_t tile_J = dim_J_padded / DIM;
  const size_t tile_K = dim_K_padded / DIM;
  ...
  //const size_t sizeof_D = low_D ? sizeof(elem_t) : sizeof(acc_t) ;
  //const size_t sizeof_C = full_C ? sizeof(acc_t) : sizeof(elem_t);
  size_t sizeof_D = sizeof(acc_t);
  size_t sizeof_C = sizeof(elem_t);

  if (config) {
    gemmini_extended_config_ex(WEIGHT_STATIONARY, 0, 0, 1, a_transpose, b_transpose);
    gemmini_extended_config_st(stride_C * sizeof_C, 0, 1);
    ...
    gemmini_extended3_config_ld((stride_D * sizeof_D), 1, false, 2);
  }
/*
  sp_tiled_matmul_ws(A, B, D, C,
          ...
          */
  // Combined loop
  gemmini_loop_ws(tile_I, tile_J, tile_K, padding_I, padding_J, padding_K, A, B, no_bias ? NULL : D, C,
    stride_A, stride_B, stride_D, stride_C,
    a_transpose, b_transpose,
    false, false, !no_bias || D == NULL,
    0, 0, 0, false, false);//A == B);
#if ReRoCC == false
  gemmini_fence();
#endif
}
...
static void sp_tiled_resadd(const size_t I, const size_t J,
    ...
    gemmini_loop_ws(tile_I, tile_J, 0, pad_I, pad_J, 0, A, B, NULL, C, A_row_stride, B_row_stride, 0, C_row_stride, false, false, false, false, false, relu, 0, 0, true, false);
    /*
...
static void tiled_resadd_small(const size_t I, const size_t J,
    ...
    sp_tiled_resadd(I, J, //I_tile, J_tile,
            A_scale, B_scale, A, B, C,//a, b, c,
...
*/
    ...
    if (matadd_type == WS) {
      tiled_resadd(I, J, A_stride, B_stride, C_stride, tile_I, tile_J,
            A_scale, B_scale, C_scale, A, B, C,
            relu, matadd_type);
    }
    else {
