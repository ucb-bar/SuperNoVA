...
#else
#define MAT_DIM_I 16 //1
#define MAT_DIM_J 48 //20
#endif

//#define MAT_DIM 32
#define NUM_ROW_TILE 2

#define TILE_ROWS (MAT_DIM_I / NUM_ROW_TILE)
#define TILE_COLS (MAT_DIM_J)
...
int main() {
    ...
    bool granted = false;
    int index = 0;
    for (int tile = 0; tile < NUM_ROW_TILE; tile ++){
        uint64_t source_addr_offset = tile * TILE_ROWS * MAT_DIM_J * sizeof(elem_t);
        uint64_t dest_addr_offset = tile * TILE_ROWS * MAT_DIM_J * sizeof(elem_t);
        dma_memcpy_multitile(channel, granted, source_addr_offset, dest_addr_offset, index, source_addr_offset, dest_addr_offset, 2, TILE_ROWS, (TILE_COLS) * sizeof(elem_t));
    }
